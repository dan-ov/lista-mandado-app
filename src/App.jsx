    ...
    <div className="min-h-screen bg-gray-50 font-sans">
      <div className="container mx-auto p-6 lg:p-8">
        <header className="text-center mb-10">
          <h1 className="text-5xl font-extrabold text-gray-800">Inventario del Hogar</h1>
          <p className="text-sm text-gray-700 mt-1">By: Daniel Olivares</p>
          <p className="text-lg text-gray-600 mt-2">Gestiona tus productos y tu lista de compras.</p>
        </header>
        ...
        <section className="max-w-3xl mx-auto mb-8">
          ...
          <ul className="space-y-2">
            {inventoryList.map(p => (
              <li key={p.id} className="flex items-center justify-between bg-white p-4 rounded shadow">
                <div>
                  <span className="font-medium">{p.name}</span>
                  <span className="ml-2 text-sm text-gray-500">({p.quantity})</span>
                </div>
        ...
        <section className="max-w-3xl mx-auto">
          ...
          <ul className="space-y-2">
            {shoppingList.map(p => (
              <li key={p.id} className="flex items-center justify-between bg-yellow-50 p-4 rounded shadow">
                <span className="font-medium">{p.name} ({p.quantity})</span>
                <button
            ...
            {shoppingList.length === 0 && (
              <p className="text-center text-gray-500">Nada en la lista de compras.</p>
            )}
          </ul>
        </section>
      </div>
